...
class AdminController extends Controller
{
    public function dashboard()
    {
        $stats = [
            ...
            'commandes_mois'   => Commande::whereMonth('created_at', now()->month)
                                          ...
                                          ->count(),
            'commandes_jour'   => Commande::whereDate('created_at', today())->count(), // ← AJOUT
            'revenus_total'    => Commande::sum('montantTotal'),
            'revenus_mois'     => Commande::whereMonth('created_at', now()->month)
                                          ->whereYear('created_at', now()->year)
                                          ->sum('montantTotal'),
            'revenus_jour'     => Commande::whereDate('created_at', today())          // ← AJOUT
                                          ->sum('montantTotal'),
            'total_clients'    => User::whereHas('role', fn($q) => $q->where('name', 'Client'))->count(),
            'clients_mois'     => User::whereHas('role', fn($q) => $q->where('name', 'Client'))
        ...
        $commandes_recentes  = Commande::with('user')
        ...
        $ventes_journalieres = $this->getVentesJournalieres(); // ← AJOUT
    ...
    private function calculerTauxConversion(): float
    {
        ...
        $avecCommande = User::whereHas('role', fn($q) => $q->where('name', 'Client'))
                            ->whereHas('commandes')
                            ->count();
    ...
    private function getVentesMensuelles(): array
    {
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            $ventes[] = [
                ...
                'montant' => (float) Commande::whereYear('created_at', $date->year)
    ...
    // ───────────────────────────────────────────
    // ──────────────────────────────────────────────────────────
    private function getVentesJournalieres(): array
    {
        $ventes = [];
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $ventes[] = [
                'jour'         => $jour,
                'montant'      => (float) Commande::whereDate('created_at', $jour)
                                                  ->sum('montantTotal') ?? 0,
                'nb_commandes' => Commande::whereDate('created_at', $jour)->count(),
            ];
        }
